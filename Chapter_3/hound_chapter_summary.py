...
def main():
    ...
    for i in range(len(chapters)):
        speech = chapter_text(chapters[i])[0]
        speech_edit = chapter_text(chapters[i]) [1]
        ...
        print("\nSUMMARY, Chapter {}:".format(i+1))
        for i in summary:
            print(i[0])

def chapter_text(chapter):
    p_elems = [element.text for element in chapter.find_all('p')]
