...
def word_length_test(words_by_author, len_shortest_corpus):
    ...
    for i, author in enumerate(words_by_author):
        word_lengths = [len(word) for word in words_by_author[author]
                        [:len_shortest_corpus]]
        by_author_length_freq_dist[author] = freqdist_freq(nltk.FreqDist(word_lengths))
...
def stopwords_test(words_by_author, len_shortest_corpus):
    ...
    for i, author in enumerate(words_by_author):
        ...
        stopwords_by_author_freq_dist[author] = freqdist_freq(nltk.FreqDist(stopwords_by_author))
...
def parts_of_speech_test(words_by_author, len_shortest_corpus):
    ...
    for i, author in enumerate(words_by_author):
        pos_by_author = [pos[1] for pos in nltk.pos_tag(words_by_author[author]
                                                        [:len_shortest_corpus])] 
        by_author_pos_freq_dist[author] = freqdist_freq(nltk.FreqDist(pos_by_author))
    ...
    for author in words_by_author:
        if author != 'unknown': 
            ...
            author_proportion = (len(words_by_author[author])/
                                 len(combined_corpus))
            combined_freq_dist = freqdist_freq(nltk.FreqDist(combined_corpus))
...
def freqdist_freq(fd):
    ...
    INPUT:
        - the FreqDist object
    OUTPUT: dictionary with normalized frequncies.
    """
